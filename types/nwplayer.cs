...
namespace NWN
{
    public class NWPlayer : NWCreature
    {
        ...
        public virtual IEnumerable<NWPlayer> PartyMembers
        {
            get
            {
                for (NWPlayer member = NWScript.GetFirstFactionMember(this).AsPlayer(); 
                    member; member = NWScript.GetNextFactionMember(this).AsPlayer())
                {
                    yield return member;
                }
            }
        ...
        void SetLikes(NWPlayer other, bool likes)
        {
            if (likes)
